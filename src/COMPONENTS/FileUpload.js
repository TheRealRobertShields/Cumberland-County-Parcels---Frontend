...
const FileUpload = () => {
    ...
    const processFile = () => {
        ...
        if (file && file.name.includes('csv')) {
            ...
            alert('I only eat .csv files (comma delimited)!')
        }
    }
    ...
    const processData = (data) => {
        ...
        // Validation!! checking that at least the first col name is 'PARCEL_PK'
        var colHeaders = lines[0].split(',')
        if (!colHeaders[0].includes('PARCEL_PK')) {
            if (colHeaders[26] && !colHeaders[26].includes('NEIGHBORHOOD')) {
                alert('.csv data not in expected format :(')
            }
            alert('.csv data definitely not in expected format :(')
        } else {
